  ...
  function ok_file($file, $number, $arg) {
    ...
    cat $file | % {
      ...
      if ($line -ne "" -and $line -ne $null) { 
        if ($line.indexOf('#') -gt 0 -or $line -notmatch '#') {
          $num = $num + 1;
          $commands.Add(("" + $num), $line);
        }
      }
    }
    Write-verbose "Number: $number Args $arg number of commands: $num"
    if ($number -ne $null -and $num -ge 1) {
      ...
      cat $file | % {
        ...
        if ($command) {
          $num = $num + 1
          $comment = If ($comment -eq $Null) { "" } else { $comment }
          Write-Host -NoNewLine "# $num. " -foregroundcolor green
          Write-Host -NoNewLIne $comment.PadRight($maxCommentLength, " ") -foregroundcolor "green"
          Write-Host $command -foregroundcolor "white"
        } else {
          #Write-Host -NoNewLine "".PadRight($maxCommandLength, "#") -foregroundcolor "green"
